...
const initialSeedData = {
  ...
  upgrades: [
    { id: "up_solar", name: "Premium Solar & Battery Package", category: "Sustainability", description: "10kW Tier-1 monocrystalline roof panels coupled with a 15kWh local battery storage bank. Provides complete grid independence, emergency backup power, and advanced energy monitoring app.", estimated_price: 1850000, active: true, image_urls: ["https://images.unsplash.com/photo-1509391366360-2e959784a276?auto=format&fit=crop&w=800&q=80"] },
    { id: "up_kitchen", name: "Chef's Kitchen Artisan Upgrade", category: "Kitchen", description: "Solid quartz countertops, solid wood custom shaker cabinets with soft-close hinges, integrated cabinet-face refrigerator, high-end commercial induction range, and smart convection oven.", estimated_price: 1250000, active: true, image_urls: ["https://images.unsplash.com/photo-1556911220-e15b29be8c8f?auto=format&fit=crop&w=800&q=80"] },
    { id: "up_deck", name: "Wrapped Redwood Cedar Deck", category: "Outdoor Living", description: "450 sq ft of custom-crafted local red cedar decking wrapping around two sides of the home. Includes black aluminum railings, built-in LED steps lighting, and structural prep for a hot tub.", estimated_price: 1400000, active: true, image_urls: ["https://images.unsplash.com/photo-1600585154526-990dced4db0d?auto=format&fit=crop&w=800&q=80"] },
    { id: "up_smarthome", name: "Smart Home Automation Hub", category: "Interior", description: "Full mesh smart home package: Nest learning thermostats, Ring video doorbell, smart locks, built-in Sonos ceiling speakers in main room, automated roller shades, and a central iPad control mount.", estimated_price: 480000, active: true, image_urls: ["https://images.unsplash.com/photo-1558002038-1055907df827?auto=format&fit=crop&w=800&q=80"] },
    { id: "up_windows", name: "Triple-Pane Passive Window Upgrade", category: "Energy Efficiency", description: "High-performance German-engineered triple-glazed, argon-filled wood-clad vinyl windows. Drastically reduces acoustic transmission, eliminates drafts, and lowers heating/cooling bills by up to 35%.", estimated_price: 920000, active: true, image_urls: ["https://images.unsplash.com/photo-1505691938895-1758d7feb511?auto=format&fit=crop&w=800&q=80"] },
    { id: "up_rainwater", name: "Rainwater Harvest & Clean Water Loop", category: "Sustainability", description: "10,000L heavy-duty underground storage tank with dual-stage commercial sediment filtration, UV sterilizer, and booster pump. Feeds all toilets, outdoor spigots, and laundry units with clean rainwater.", estimated_price: 750000, active: true, image_urls: ["https://images.unsplash.com/photo-1518837695005-2083093ee35b?auto=format&fit=crop&w=800&q=80"] },
    { id: "up_bathroom", name: "Luxury Walk-in Spa Wetroom", category: "Bathroom", description: "Curbless walk-in shower with floor-to-ceiling porcelain tiles, rain-shower head, high-efficiency custom linear floor drains, heated bathroom floor tiles, and a floating organic oak vanity.", estimated_price: 620000, active: true, image_urls: ["https://images.unsplash.com/photo-1552321554-5fefe8c9ef14?auto=format&fit=crop&w=800&q=80"] },
    { id: "up_furniture", name: "Turnkey Scandinavian Furniture Package", category: "Rental/Furnishing Package", description: "Curated modern furniture selection matching the home model: oak dining table and chairs, comfortable linen sofa, master queen bed frame, high-density mattresses, custom window drapery, and art accents.", estimated_price: 1600000, active: true, image_urls: ["https://images.unsplash.com/photo-1586023492125-27b2c045efd7?auto=format&fit=crop&w=800&q=80"] }
  ],
  design_upgrades: [
...
const db = {
  // Initialize Database
  init() {
    const dataStr = safeStorage.getItem(DB_KEY);
    if (!dataStr) {
      ...
      // Dynamic migration logic to inject missing collections from schema upgrades
      try {
        const data = JSON.parse(dataStr);
        let updated = false;
        
        for (const key in initialSeedData) {
          if (!data[key]) {
            if (Array.isArray(initialSeedData[key])) {
              data[key] = [...initialSeedData[key]];
            } else {
              data[key] = { ...initialSeedData[key] };
            }
            updated = true;
            console.log(`Database Migration: Seeded missing collection [${key}].`);
          }
        }
        
        if (data.upgrades) {
          data.upgrades.forEach(up => {
            if (!up.image_urls || !Array.isArray(up.image_urls)) {
              const defaultImages = {
                up_solar: ["https://images.unsplash.com/photo-1509391366360-2e959784a276?auto=format&fit=crop&w=800&q=80"],
                up_kitchen: ["https://images.unsplash.com/photo-1556911220-e15b29be8c8f?auto=format&fit=crop&w=800&q=80"],
                up_deck: ["https://images.unsplash.com/photo-1600585154526-990dced4db0d?auto=format&fit=crop&w=800&q=80"],
                up_smarthome: ["https://images.unsplash.com/photo-1558002038-1055907df827?auto=format&fit=crop&w=800&q=80"],
                up_windows: ["https://images.unsplash.com/photo-1505691938895-1758d7feb511?auto=format&fit=crop&w=800&q=80"],
                up_rainwater: ["https://images.unsplash.com/photo-1518837695005-2083093ee35b?auto=format&fit=crop&w=800&q=80"],
                up_bathroom: ["https://images.unsplash.com/photo-1552321554-5fefe8c9ef14?auto=format&fit=crop&w=800&q=80"],
                up_furniture: ["https://images.unsplash.com/photo-1586023492125-27b2c045efd7?auto=format&fit=crop&w=800&q=80"]
              };
              up.image_urls = defaultImages[up.id] || [];
              updated = true;
              console.log(`Database Migration: Injected missing image_urls list for upgrade [${up.id}].`);
            }
          });
        }
  ...
  saveSiteSettings(settings) {
    const data = this._read();
    data.site_settings = { ...this.getSiteSettings(), ...settings };
    this._write(data);
    return data.site_settings;
  },
  ...
  saveFinishPackage(pkg) {
    const data = this._read();
    if (!data.finish_packages) {
      data.finish_packages = [];
    }
    if (!pkg.id) {
      pkg.id = generateUUID();
